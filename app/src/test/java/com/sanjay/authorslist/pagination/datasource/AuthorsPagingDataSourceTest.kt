...
class AuthorsPagingDataSourceTest {

    private var pagingDataSource: AuthorsPagingDataSource? = null
    ...
    @Mock
    lateinit var loadCallback: PageKeyedDataSource.LoadCallback<Int, Author>

    private var loadInitialParams = PageKeyedDataSource.LoadInitialParams<Int>(20, false)

    @Mock
    lateinit var loadInitialCallback: PageKeyedDataSource.LoadInitialCallback<Int, Author>
    ...
    fun setUp() {
        ...
        pagingDataSource = AuthorsPagingDataSource(repository)
    }
    ...
    fun loadInitial_Success() {
        ...
        whenever(repository.getAuthors(1, loadInitialParams.requestedLoadSize)).thenReturn(
            observable
        )
        ...
        verify(repository).getAuthors(1, loadInitialParams.requestedLoadSize)
        verify(loadInitialCallback).onResult(authorsList, null, 2)
        argumentCaptor.run {
    ...
    fun loadInitial_Retry() {
        ...
        verify(repository, times(2)).getAuthors(1, loadInitialParams.requestedLoadSize)
    }
    ...
    fun loadAfter_Success() {
        ...
        whenever(repository.getAuthors(loadParams.key, loadParams.requestedLoadSize)).thenReturn(
            observable
        )
        ...
        verify(repository).getAuthors(loadParams.key, loadParams.requestedLoadSize)
    ...
    fun loadAfter_Retry() {
        ...
        verify(repository, times(2)).getAuthors(loadParams.key, loadParams.requestedLoadSize)
    }
}