...
class PostsPagingDataSource @Inject constructor(private val repository: AuthorsRepository) :
    ...
    override fun loadInitial(
        ...
        //Call api
        repository.getPosts(selectedAuthor.value!!.id, 1, params.requestedLoadSize)
            .subscribe(
                { posts ->
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, Post>) {
        ...
        val nextPage = currentPage + 1
        //Call api
        repository.getPosts(selectedAuthor.value!!.id, currentPage, params.requestedLoadSize)
            .subscribe(
                { posts ->
                    updateState(State.DONE)
